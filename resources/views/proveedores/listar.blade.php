...
@section('content')
    ...
    <div id="content" id="content" class="bg-white/10 col-span-9 rounded-lg p-0">
        <div class="w-full ">
            <div class="bg-white shadow-md rounded pt-1 pl-1 pr-1 ">
                ...
                <button type="button"
                    class=" bg-indigo-300 h-max w-max rounded-lg text-white font-bold hover:bg-indigo-500 justify-end "
                    disabled>
                ...
                <h1 class="font-bold py-4 uppercase">Lista de proveedores</h1>
                            ...
                            @foreach ($proveedores as $proveedor)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        ...
                                        {{ ($proveedor->tipo_doc ?? '1') == '1' ? 'RUC' : 'DNI' }}


                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"> {{ $proveedor->ruc }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $proveedor->direccion }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $proveedor->telefono }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <div class="flex item-center justify-center">

                                            <div onclick="editModal({{ $proveedor->id }})"
                                            ...
                                            <div onclick="deletModalProv({{ $proveedor->id }})"
    ...
    <dialog class="bg-white shadow-md rounded-lg max-w-4xl mx-auto p-6 " id="myModal">
        <h2 class="text-2xl font-bold text-blue-500 mb-6">Nuevo Proveedor</h2>
        <form action="{{ route('registrar.proveedor') }}" method="POST">
            ...
            <div class="grid grid-cols-2 gap-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2" for="numero">RUC/DNI *</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2" for="codigo-barra">Razón social</label>
        ...
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            ...
            <div class="sm:flex sm:items-start">
                ...
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-headline"> Eliminar Proveedor
                    </h3>
                    <div class="mt-2">
                        <p class="text-sm text-gray-500"> ¿Estas seguro de eliminar <span class="font-bold">Este
                                proveedor</span>? Esta acción no se puede deshacer. </p>
                    </div>
                </div>
            </div>
        </div>
        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <form id="deleteForm" method="POST" action="{{ route('proveedores.destroy') }}">
                @csrf
                @method('DELETE')
                <input type="text" id="proveedor_id" name="proveedor_id" hidden>
    ...
    <dialog class="bg-white shadow-md rounded-lg max-w-4xl mx-auto p-6 " id="editModal">
        <h2 class="text-2xl font-bold text-blue-500 mb-6">Editar Proveedor</h2>
        <form action="{{ route('update.proveedor') }}" id="editForm" method="POST">
            @csrf
            <div class="grid grid-cols-2 gap-6">
                <div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2" for="codigo-barra">Razón social</label>
                    <input type="text" id="rsE" name="rsE" value="{{ old('rsE') }}"
                        class="flex-grow p-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 w-full">
    ...
    <!-- Bootstrap 4 -->
    <script>
        function deletModalProv(proveedorId) {
    ...
    <script>
        ...
        function editModal(proveedorId) {
            document.getElementById('editModal').showModal();

            fetch(`/proveedores/${proveedorId}`)
                .then(response => response.json())
                .then(proveedor => {
                    // Rellenar el formulario con los datos del proveedoro
                    document.getElementById('idE').value = proveedor.id;
                    document.getElementById('numeroE').value = proveedor.ruc;
                    document.getElementById('rsE').value = proveedor.razon_social;
                    document.getElementById('nombre_comercialE').value = proveedor.nombre_comercial;
                    document.getElementById('direccionE').value = proveedor.direccion;
                    document.getElementById('telefonoE').value = proveedor.telefono;
                    updateSelect('tipo_docE', proveedor.tipo_doc);
